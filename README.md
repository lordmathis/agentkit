...
AgentKit exposes a REST API for integration and management:

### Models & Providers
- `GET /api/models` - List all available models (chatbots + provider models)
- `GET /api/chatbots` - List chatbot plugins with their configurations
- `GET /api/providers` - List all configured providers and their models

### Configuration
- `GET /api/config/default-chat` - Get default chat configuration

### Tools
- `GET /api/tools` - List all tool servers and their available tools

### Chats
- `/api/chats/*` - Chat management endpoints (create, list, delete conversations)

### Skills
- `/api/skills/*` - Skill management endpoints

### GitHub Integration
- `/api/github/*` - GitHub integration endpoints (requires `github_token` configuration)

### Transcription
- `POST /api/transcribe` - Transcribe audio files (requires `transcription` configuration)
