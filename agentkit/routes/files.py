...
async def upload_github_files(
    request: Request,
    github_req: GitHubFilesRequest
):
    ...
    try:
        file_paths = await _expand_paths_to_files(
            ...
            github_req.repo,
            github_req.paths,
            github_req.exclude_paths
        )
    except Exception as e:
        logger.error(f"Failed to expand GitHub paths: {e}")
        raise HTTPException(status_code=400, detail=f"Failed to expand GitHub paths: {e}")
    ...
    for path in file_paths:
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Failed to download GitHub file {path}: {e}")
