...
async def estimate_tokens(request: Request, body: EstimateTokensRequest):
    ...
    try:
        ...
        all_file_paths = await _expand_paths_to_files(
            github_client, body.repo, body.paths, body.exclude_paths or []
        )
...
async def _expand_paths_to_files(
    github_client, repo: str, paths: List[str], exclude_paths: List[str]
) -> List[str]:
        ...
        try:
            ...
            if tree_node.type == "file":
                ...
                files_in_dir = await _get_all_files_in_dir(
                    github_client, repo, tree_node, exclude_set
                )
...
async def _get_all_files_in_dir(
    github_client, repo: str, node, exclude_set: set
) -> List[str]:
        ...
        if child.type == "file":
            ...
            try:
                ...
                subfiles = await _get_all_files_in_dir(
                    github_client, repo, subtree, exclude_set
                )
